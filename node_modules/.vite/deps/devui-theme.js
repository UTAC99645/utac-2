//#region node_modules/devui-theme/index.es.js
...
function getUrls(urls) {
	...
	function onSuccess2(responseText, urlIndex) {
		...
		if (urlQueue.indexOf(null) === -1) settings.onComplete(urlQueue);
	}
	var parser = document.createElement("a");
	urlArray.forEach(function(url, i) {
		parser.setAttribute("href", url);
		parser.href = String(parser.href);
		if (Boolean(document.all && !window.atob) && parser.host.split(":")[0] !== location.host.split(":")[0]) if (parser.protocol === location.protocol) {
			...
			xdr.onload = function() {
				...
				if (isValidCss(text)) onSuccess2(text, i);
				else onError2(xdr, i);
			};
			...
			xhr.open("GET", url);
			if (settings.mimeType && xhr.overrideMimeType) xhr.overrideMimeType(settings.mimeType);
			settings.onBeforeSend(xhr, url, i);
			xhr.onreadystatechange = function() {
				if (xhr.readyState === 4) {
					var text = xhr.responseText;
					if (xhr.status < 400 && isValidCss(text)) onSuccess2(text, i);
					else if (xhr.status === 0 && isValidCss(text)) onSuccess2(text, i);
					else onError2(xhr, i);
				}
			};
			xhr.send();
...
function getCssData(options) {
	...
	function handleComplete() {
		if (cssArray.indexOf(null) === -1) {
			cssArray.reduce(function(skipIndices, value, i) {
				if (value === "") skipIndices.push(i);
				return skipIndices;
	...
	function handleSuccess(cssText, cssIndex, node, sourceUrl) {
		...
		resolveImports(cssText, node, sourceUrl, function(resolvedCssText, errorData) {
			if (cssArray[cssIndex] === null) {
				...
				if (!settings.filter || settings.filter.test(resolvedCssText)) cssArray[cssIndex] = resolvedCssText;
				else cssArray[cssIndex] = "";
				handleComplete();
			}
		});
	}
	...
	function resolveImports(cssText, node, baseUrl, callbackFn) {
		...
		if (importData.rules.length) getUrls(importData.absoluteUrls, {
		...
		else callbackFn(cssText, __errorData);
	}
	if (sourceNodes.length) sourceNodes.forEach(function(node, i) {
		var linkHref = node.getAttribute("href");
		var linkRel = node.getAttribute("rel");
		var isLink = node.nodeName.toLowerCase() === "link" && linkHref && linkRel && linkRel.toLowerCase().indexOf("stylesheet") !== -1;
		var isSkip = settings.skipDisabled === false ? false : node.disabled;
		var isStyle = node.nodeName.toLowerCase() === "style";
		if (isLink && !isSkip) getUrls(linkHref, {
			mimeType: "text/css",
			onBeforeSend: function onBeforeSend2(xhr, url, urlIndex) {
				settings.onBeforeSend(xhr, node, url);
			},
			onSuccess: function onSuccess2(cssText2, url, urlIndex) {
				handleSuccess(cssText2, i, node, getFullUrl$1(linkHref));
			},
			onError: function onError2(xhr, url, urlIndex) {
				cssArray[i] = "";
			...
			if (settings.useCSSOM) cssText = Array.apply(null, node.sheet.cssRules).map(function(rule) {
				return rule.cssText;
			}).join("");
...
function parseCss(css) {
	...
	function comments() {
		var cmnts = [];
		var c;
		while (c = comment()) cmnts.push(c);
		return settings.removeComments ? [] : cmnts;
	}
	function selector() {
		whitespace();
		while (css[0] === "}") error("extra closing bracket");
		var m = match(/^(("(?:\\"|[^"])*"|'(?:\\'|[^'])*'|[^{])+)/);
		if (m) {
			var _selector = m[0].trim();
			var selectorItems;
			if (/\/\*/.test(_selector)) _selector = _selector.replace(/\/\*([^*]|[\r\n]|(\*+([^*/]|[\r\n])))*\*\/+/g, "");
			var hasCommaInQuotes = /["']\w*,\w*["']/.test(_selector);
			if (hasCommaInQuotes) _selector = _selector.replace(/"(?:\\"|[^"])*"|'(?:\\'|[^'])*'/g, function(m2) {
				return m2.replace(/,/g, "‌");
			});
			if (/,/.test(_selector)) selectorItems = _selector.split(/\s*(?![^(]*\)),\s*/);
			else selectorItems = [_selector];
			if (hasCommaInQuotes) selectorItems = selectorItems.map(function(s) {
				return s.replace(/\u200C/g, ",");
			});
	...
	function at_page_margin_box() {
		...
		if (m) return {
			...
			name: "".concat(m[1], "-").concat(m[2]) + (m[3] ? "-".concat(m[3]) : ""),
			declarations: declarations()
		};
	}
	...
	function at_rule() {
		...
		if (css[0] === "@") {
			...
			if (ret && !settings.preserveStatic) {
				...
				if (ret.declarations) hasVarFunc = ret.declarations.some(function(decl) {
					return /var\(/.test(decl.value);
				});
				else hasVarFunc = (ret.keyframes || ret.rules || []).some(function(obj) {
...
function parseVars(cssData) {
	...
	if (typeof cssData === "string") cssData = parseCss(cssData, settings);
	cssData.stylesheet.rules.forEach(function(rule) {
		...
		rule.declarations.forEach(function(decl, i) {
			...
			if (prop && prop.indexOf("--") === 0) settings.store[prop] = value;
		});
	});
...
function stringifyCss(tree) {
	...
	var renderMethods = {
		...
		rule: function rule(node) {
			...
			if (decls.length) return node.selectors.join(",") + "{" + visit(decls) + "}";
		},
	...
	function visit(nodes) {
		...
		for (var i = 0; i < nodes.length; i++) {
			...
			if (txt) {
				buf += txt;
				if (txt.length && n.selectors) buf += delim;
			}
		}
		return buf;
	}
	...
	node.rules.forEach(function(rule) {
		...
		if (rule.keyframes) {
			rule.keyframes.forEach(function(keyframe) {
				if (keyframe.type === "keyframe") fn(keyframe.declarations, rule);
			});
			return;
		}
		...
		for (var i = 0; i < declarations.length; i++) {
			...
			if (value.indexOf(VAR_FUNC_IDENTIFIER + "(") !== -1) {
				...
				if (resolvedValue !== decl.value) {
					...
					if (!settings.preserveVars) decl.value = resolvedValue;
					else {
...
function fixNestedCalc(value) {
	(value.match(/calc\(([^)]+)\)/g) || []).forEach(function(match) {
...
function resolveValue(value) {
	...
	function resolveFunc(value2) {
		var name$1 = value2.split(",")[0].replace(/[\s\n\t]/g, "");
		var fallback = (value2.match(/(?:\s*,\s*){1}(.*)?/) || [])[1];
		var match = Object.prototype.hasOwnProperty.call(settings.variables, name$1) ? String(settings.variables[name$1]) : void 0;
		var replacement = match || (fallback ? String(fallback) : void 0);
		var unresolvedFallback = __recursiveFallback || value2;
		if (!match) settings.onWarning("variable \"".concat(name$1, "\" is undefined"));
		if (replacement && replacement !== "undefined" && replacement.length > 0) return resolveValue(replacement, settings, unresolvedFallback);
		else return "var(".concat(unresolvedFallback, ")");
	}
	if (!valueData) {
		if (value.indexOf("var(") !== -1) settings.onWarning("missing closing \")\" in the value \"".concat(value, "\""));
		return value;
	} else if (valueData.pre.slice(-3) === "var") if (valueData.body.trim().length === 0) {
		settings.onWarning("var() must contain a non-whitespace string");
		return value;
	} else return valueData.pre.slice(0, -3) + resolveFunc(valueData.body) + resolveValue(valueData.post, settings);
	else return valueData.pre + "(".concat(resolveValue(valueData.body, settings), ")") + resolveValue(valueData.post, settings);
}
...
function cssVars() {
	...
	function handleError(message, sourceNode, xhr, url) {
		if (!settings.silent && window.console) console.error("".concat(msgPrefix).concat(message, "\n"), sourceNode);
		settings.onError(message, sourceNode, xhr, url);
	}
	function handleWarning(message) {
		if (!settings.silent && window.console) console.warn("".concat(msgPrefix).concat(message));
	...
	if (!settings.__benchmark) {
		...
		if (!cssVarsObserver) {
			[].slice.call(settings.rootElement.querySelectorAll("[data-cssvars=\"out\"]")).forEach(function(outNode) {
				var dataGroup = outNode.getAttribute("data-cssvars-group");
				if (!(dataGroup ? settings.rootElement.querySelector("[data-cssvars=\"src\"][data-cssvars-group=\"".concat(dataGroup, "\"]")) : null)) outNode.parentNode.removeChild(outNode);
			});
	...
	if (document.readyState !== "loading") if (isNativeSupport && settings.onlyLegacy) {
		var hasVarChange = false;
		if (settings.updateDOM) {
			var targetElm = settings.rootElement.host || (settings.rootElement === document ? document.documentElement : settings.rootElement);
			Object.keys(settings.variables).forEach(function(key$1) {
				var varValue = settings.variables[key$1];
				hasVarChange = hasVarChange || varValue !== getComputedStyle(targetElm).getPropertyValue(key$1);
				targetElm.style.setProperty(key$1, varValue);
			});
		}
		handleFinally(hasVarChange);
	} else if (!isShadowDOMReady && (settings.shadowDOM || settings.rootElement.shadowRoot || settings.rootElement.host)) getCssData({
		rootElement: defaults.rootElement,
		include: defaults.include,
		exclude: settings.exclude,
		skipDisabled: false,
		onSuccess: function onSuccess2(cssText, node, url) {
			if ((node.sheet || {}).disabled && !node.__cssVars) return false;
		...
		getCssData({
			...
			onError: function onError2(xhr, node, url) {
				...
				handleError("CSS XHR Error: ".concat(responseUrl, " ").concat(xhr.status, " ").concat(statusText), node, xhr, responseUrl);
			},
			onSuccess: function onSuccess2(cssText, node, url) {
				if ((node.sheet || {}).disabled && !node.__cssVars) return false;
				...
				cssText = returnVal !== void 0 && Boolean(returnVal) === false ? "" : returnVal || cssText;
				if (settings.updateURLs && (isLink || isStyleImport)) cssText = fixRelativeCssUrls(cssText, url);
				return cssText;
			},
			onComplete: function onComplete2(cssText, cssArray) {
				...
				nodeArray.forEach(function(node, i2) {
					...
					if (regex.cssVars.test(nodeCSS)) try {
				...
				hasVarChange2 = counters.job > 0 && Boolean(Object.keys(variableStore.job).length > Object.keys(currentVars).length || Boolean(Object.keys(currentVars).length && Object.keys(variableStore.job).some(function(key$1) {
					return variableStore.job[key$1] !== currentVars[key$1];
				})));
				if (hasVarChange2) {
					...
					if (settings.updateDOM) counters.job++;
					nodeArray.forEach(function(node, i2) {
						var isSkip = !node.__cssVars.tree;
						if (node.__cssVars.tree) try {
							...
							if (settings.updateDOM) {
								...
								if (!node.getAttribute("data-cssvars")) node.setAttribute("data-cssvars", "src");
								if (outCss.length && hasCSSVarFunc) {
									var dataGroup = node.getAttribute("data-cssvars-group") || ++counters.group;
									var outCssNoSpaces = outCss.replace(/\s/g, "");
									var outNode = settings.rootElement.querySelector("[data-cssvars=\"out\"][data-cssvars-group=\"".concat(dataGroup, "\"]")) || document.createElement("style");
									hasKeyframesWithVars = hasKeyframesWithVars || regex.cssKeyframes.test(outCss);
									if (settings.preserveStatic) node.sheet && (node.sheet.disabled = true);
									...
									if (outCssNoSpaces === node.textContent.replace(/\s/g, "")) {
										...
										if (!outNode.parentNode) node.parentNode.insertBefore(outNode, node.nextSibling);
									}
								}
							} else if (node.textContent.replace(/\s/g, "") !== outCss) outCssArray.push(outCss);
						} catch (err) {
							handleError(err.message, node);
						}
						if (isSkip) node.setAttribute("data-cssvars", "skip");
						if (!node.hasAttribute("data-cssvars-job")) node.setAttribute("data-cssvars-job", counters.job);
					});
					cssVarsSrcNodeCount = settings.rootElement.querySelectorAll("[data-cssvars]:not([data-cssvars=\"out\"])").length;
					if (settings.shadowDOM) {
						var elms = [].concat(settings.rootElement).concat([].slice.call(settings.rootElement.querySelectorAll("*")));
						for (var i = 0, elm; elm = elms[i]; ++i) if (elm.shadowRoot && elm.shadowRoot.querySelector("style")) cssVars(_extends({}, settings, { rootElement: elm.shadowRoot }));
					}
	...
	else document.addEventListener("DOMContentLoaded", function init(evt) {
		cssVars(options);
...
cssVars.reset = function() {
	...
	for (var prop in variableStore) variableStore[prop] = {};
};
function addMutationObserver(settings) {
	...
	function isLink(node) {
		return node.nodeName.toLowerCase() === "link" && (node.getAttribute("rel") || "").indexOf("stylesheet") !== -1;
	}
	...
	function isValidAttributeMutation(mutation) {
		...
		if (mutation.type === "attributes" && isLink(mutation.target) && !isDisabled(mutation.target)) {
			...
			if (isEnabledMutation) isValid = !isSkipNode && !isSrcNode;
			else if (isHrefMutation) {
				if (isSkipNode) mutation.target.setAttribute("data-cssvars", "");
				else if (isSrcNode) resetCssNodes(settings.rootElement, true);
				isValid = true;
			}
		}
		return isValid;
	...
	function isValidAddMutation(mutation) {
		var isValid = false;
		if (mutation.type === "childList") isValid = [].slice.call(mutation.addedNodes).some(function(node) {
			var hasAttr = node.nodeType === 1 && node.hasAttribute("data-cssvars");
			var isStyleWithVars = isStyle(node) && regex.cssVars.test(node.textContent);
			return !hasAttr && (isLink(node) || isStyleWithVars) && !isDisabled(node);
		});
		return isValid;
	}
	function isValidRemoveMutation(mutation) {
		var isValid = false;
		if (mutation.type === "childList") isValid = [].slice.call(mutation.removedNodes).some(function(node) {
			...
			if (isSrcNode || isOutNode) {
				...
				if (isSrcNode) resetCssNodes(settings.rootElement, true);
...
function fixKeyframes(rootElement) {
	...
	if (animationNameProp) {
		...
		for (var i = 0, len = allNodes.length; i < len; i++) {
			var node = allNodes[i];
			if (getComputedStyle(node)[animationNameProp] !== "none") {
...
function fixRelativeCssUrls(cssText, baseUrl) {
	(cssText.replace(regex.cssComments, "").match(regex.cssUrls) || []).forEach(function(cssUrl) {
...
function fixVarNames() {
	...
	return Object.keys(varObj).reduce(function(obj, value) {
		var key$1 = reLeadingHyphens.test(value) ? value : "--".concat(value.replace(/^-+/, ""));
		obj[key$1] = varObj[value];
		return obj;
	}, {});
}
...
function resetCssNodes(rootElement) {
	...
	[].slice.call(rootElement.querySelectorAll("[data-cssvars=\"skip\"],[data-cssvars=\"src\"]")).forEach(function(node) {
		return node.setAttribute("data-cssvars", "");
	});
	if (resetDOMVariableStore) variableStore.dom = {};
}
var THEME_KEY = {
...
var ColorHierarchyMap = {
	"devui-base-bg": { "default-value": "#ffffff" },
	"devui-base-bg-dark": { "default-value": "#333854" },
	"devui-global-bg": { "extends": "devui-base-bg" },
	"devui-global-bg-normal": { "extends": "devui-base-bg" },
	"devui-brand": {
	...
	"devui-contrast": { "default-value": "#c7000a" },
	"devui-brand-foil": { "extends": "devui-brand" },
	"devui-brand-hover": { "extends": "devui-brand" },
	"devui-brand-active": { "extends": "devui-brand" },
	"devui-brand-active-focus": { "extends": "devui-brand-active" },
	"devui-text": { "default-value": "#252b3a" },
	"devui-text-weak": { "extends": "devui-text" },
	"devui-aide-text": { "extends": "devui-text" },
	"devui-aide-text-stress": { "extends": "devui-aide-text" },
	"devui-placeholder": { "extends": "devui-text" },
	"devui-light-text": { "extends": "devui-text" },
	"devui-dark-text": { "extends": "devui-text" },
	"devui-link": { "extends": "devui-brand" },
	"devui-link-active": { "extends": "devui-link" },
	"devui-link-light": { "extends": "devui-link" },
	"devui-link-light-active": { "extends": "devui-link-light" },
	"devui-line": { "default-value": "#adb0b8" },
	"devui-dividing-line": { "extends": "devui-line" },
	"devui-block": { "extends": "devui-base-bg" },
	"devui-area": { "extends": "devui-block" },
	"devui-danger": { "default-value": "#f66f6a" },
	"devui-warning": { "default-value": "#fac20a" },
	"devui-waiting": { "default-value": "#9faad7" },
	"devui-success": { "default-value": "#50d4ab" },
	"devui-info": { "default-value": "#5e7ce0" },
	"devui-initial": { "default-value": "#e9edfa" },
	"devui-unavailable": { "default-value": "#f5f5f6" },
	"devui-shadow": { "default-value": "rgba(0, 0, 0, 0.2)" },
	"devui-light-shadow": { "extends": "devui-shadow" },
	"devui-icon-text": { "extends": "devui-text" },
	"devui-icon-bg": { "extends": "devui-block" },
	"devui-icon-fill": { "default-value": "#d3d5d9" },
	"devui-icon-fill-hover": {
		"extends": "devui-brand",
		"dependency": "devui-icon-fill"
	},
	"devui-icon-fill-active": { "extends": "devui-brand" },
	"devui-icon-fill-active-hover": { "extends": "devui-icon-fill-active" },
	"devui-form-control-line": { "extends": "devui-line" },
	"devui-form-control-line-hover": { "extends": "devui-form-control-line" },
	"devui-form-control-line-active": { "extends": "devui-brand" },
	"devui-form-control-line-active-hover": { "extends": "devui-form-control-line-active" },
	"devui-list-item-active-bg": { "extends": "devui-brand" },
	"devui-list-item-active-text": { "extends": "devui-light-text" },
	"devui-list-item-active-hover-bg": { "extends": "devui-list-item-active-bg" },
	"devui-list-item-hover-text": { "extends": "devui-brand-active" },
	"devui-list-item-hover-bg": { "extends": "devui-brand-active" },
	"devui-list-item-selected-bg": { "extends": "devui-brand-active" },
	"devui-list-item-strip-bg": { "extends": "devui-brand-active" },
	"devui-disabled-bg": { "extends": "devui-base-bg" },
	"devui-disabled-line": {
	...
	"devui-primary-disabled": { "extends": "devui-primary" },
	"devui-icon-fill-active-disabled": { "extends": "devui-icon-fill-active" },
	"devui-label-bg": {
		"extends": "devui-base-bg",
		"dependency": "devui-brand"
	},
	"devui-connected-overlay-bg": { "extends": "devui-base-bg" },
	"devui-connected-overlay-line": { "extends": "devui-brand" },
	"devui-fullscreen-overlay-bg": { "extends": "devui-base-bg" },
	"devui-feedback-overlay-bg": { "default-value": "#464d6e" },
	"devui-feedback-overlay-text": { "extends": "devui-light-text" },
	"devui-embed-search-bg": {
		"extends": "devui-base-bg",
		"dependency": "devui-brand"
	},
	"devui-embed-search-bg-hover": { "extends": "devui-embed-search-bg" },
	"devui-float-block-shadow": { "extends": "devui-brand" },
	"devui-highlight-overlay": { "extends": "devui-base-bg" },
	"devui-range-item-hover-bg": {
		"extends": "devui-base-bg",
		"dependency": "devui-brand"
	},
	"devui-primary": { "extends": "devui-brand" },
	"devui-primary-hover": { "extends": "devui-primary" },
	"devui-primary-active": { "extends": "devui-primary" },
	"devui-contrast-hover": { "extends": "devui-contrast" },
	"devui-contrast-active": { "extends": "devui-contrast" },
	"devui-danger-line": { "extends": "devui-danger" },
	"devui-danger-bg": { "extends": "devui-danger" },
	"devui-warning-line": { "extends": "devui-warning" },
	"devui-warning-bg": { "extends": "devui-warning" },
	"devui-info-line": { "extends": "devui-info" },
	"devui-info-bg": { "extends": "devui-info" },
	"devui-success-line": { "extends": "devui-success" },
	"devui-success-bg": { "extends": "devui-success" },
	"devui-primary-line": { "extends": "devui-primary" },
	"devui-primary-bg": { "extends": "devui-primary" },
	"devui-default-line": { "extends": "devui-brand" },
	"devui-default-bg": { "extends": "devui-base-bg" }
};
...
var isArrayish2 = function isArrayish(obj) {
...
var swizzle$1 = simpleSwizzle.exports = function swizzle(args) {
	...
	for (var i = 0, len = args.length; i < len; i++) {
		var arg = args[i];
		if (isArrayish2(arg)) results = concat.call(results, slice.call(arg));
...
for (var model in convert$2) if (convert$2.hasOwnProperty(model)) {
...
convert$2.hex.rgb = function(args) {
	...
	if (match[0].length === 3) colorString2 = colorString2.split("").map(function(char) {
		return char + char;
	}).join("");
...
function buildGraph() {
	...
	for (var len = models2.length, i = 0; i < len; i++) graph[models2[i]] = {
		distance: -1,
		parent: null
	};
	return graph;
...
var hashedModelKeys = {};
Object.keys(convert).forEach(function(model$1) {
	hashedModelKeys[_slice.call(convert[model$1].labels).sort().join("")] = model$1;
});
var limiters = {};
function Color(obj, model$1) {
	...
	if (model$1 && !(model$1 in convert)) throw new Error("Unknown model: " + model$1);
	var i;
	var channels$1;
	if (obj == null) {
		this.model = "rgb";
		...
		channels$1 = convert[this.model].channels;
		this.color = result.value.slice(0, channels$1);
		this.valpha = typeof result.value[channels$1] === "number" ? result.value[channels$1] : 1;
	} else if (obj.length) {
		this.model = model$1 || "rgb";
		channels$1 = convert[this.model].channels;
		this.color = zeroArray(_slice.call(obj, 0, channels$1), channels$1);
		this.valpha = typeof obj[channels$1] === "number" ? obj[channels$1] : 1;
		...
		var labels$1 = convert[this.model].labels;
		var color2 = [];
		for (i = 0; i < labels$1.length; i++) color2.push(obj[labels$1[i]]);
...
Color.prototype = {
	...
	object: function() {
		var result = {};
		var channels$1 = convert[this.model].channels;
		var labels$1 = convert[this.model].labels;
		for (var i = 0; i < channels$1; i++) result[labels$1[i]] = this.color[i];
...
Object.keys(convert).forEach(function(model$1) {
	...
	var channels$1 = convert[model$1].channels;
	Color.prototype[model$1] = function() {
		...
		var newAlpha = typeof arguments[channels$1] === "number" ? channels$1 : this.valpha;
		return new Color(assertArray(convert[this.model][model$1].raw(this.color)).concat(newAlpha), model$1);
	};
	Color[model$1] = function(color2) {
		if (typeof color2 === "number") color2 = zeroArray(_slice.call(arguments), channels$1);
		return new Color(color2, model$1);
	};
});
...
function getset(model$1, channel, modifier) {
	model$1 = Array.isArray(model$1) ? model$1 : [model$1];
	model$1.forEach(function(m) {
	...
	return function(val) {
		...
		result = this[model$1]().color[channel];
...
var color = Color;
var CustomThemeUtils = class CustomThemeUtils {
	...
	static genColorPattern(colorHierarchy, themeData) {
		const pattern = {};
		CustomThemeUtils.getThemeOffset(colorHierarchy, themeData).forEach((obj) => {
	...
	static getThemeOffset(colorHierarchy, themeData) {
		return Object.keys(colorHierarchy).map((key$1) => ({
			colorName: key$1,
			color: color(themeData[key$1]),
			extends: colorHierarchy[key$1].extends ? color(themeData[colorHierarchy[key$1].extends]) : null
		})).map((colorObj) => {
			if (colorObj.extends) colorObj.offset = {
	...
	static getColorOffset(target, source, mode) {
		...
		const key$1 = mode.split("");
		const offset = {
			[key$1[0]]: targetModel.color[0] - sourceModel.color[0],
			[key$1[1]]: targetModel.color[1] - sourceModel.color[1],
			[key$1[2]]: targetModel.color[2] - sourceModel.color[2],
			a: targetModel.valpha - sourceModel.valpha
		};
		const percent = {
			[key$1[1] + "p"]: offset[key$1[1]] > 0 ? offset[key$1[1]] * 100 / (100 - sourceModel.color[1]) : offset[key$1[1]] * 100 / sourceModel.color[1],
			[key$1[2] + "p"]: offset[key$1[2]] > 0 ? offset[key$1[2]] * 100 / (100 - sourceModel.color[2]) : offset[key$1[2]] * 100 / sourceModel.color[2],
	...
	static updateColor(colorChanges, colorHierarchy, effect) {
		...
		while (changeStack.length) {
			...
			if (count < changeKeys.length) pattern[handleKey]["default-value"] = colorChanges[count].color;
			else {
			...
			colorKeys.forEach((colorName2) => {
				if (handleKey === pattern[colorName2].extends) {
					if (!(changeStack.indexOf(colorName2) > -1) && !(changeKeys.indexOf(colorName2) > -1)) changeStack.push(colorName2);
				}
			});
			count++;
		}
	...
	static fillEmptyColor(pattern, effect) {
		Object.keys(pattern).map((colorName2) => ({
			colorName: colorName2,
			pattern: pattern[colorName2]
		})).filter((color2) => !color2.pattern["default-value"]).forEach((color$1) => {
		...
		switch (effect) {
			...
			case "strong":
				result = {
					...
					offset: {
						...colorOffset.hsl,
						sp: ((_a = colorOffset == null ? void 0 : colorOffset.hsl) == null ? void 0 : _a.sp) > 0 ? CustomThemeUtils.minmax(((_b = colorOffset == null ? void 0 : colorOffset.hsl) == null ? void 0 : _b.sp) * 1.3, (_c = colorOffset == null ? void 0 : colorOffset.hsl) == null ? void 0 : _c.sp, 100) : ((_d = colorOffset == null ? void 0 : colorOffset.hsl) == null ? void 0 : _d.sp) * .75
					}
				};
				break;
			case "soft":
				result = {
					...
					offset: {
						...colorOffset.hsv,
						sp: ((_e = colorOffset == null ? void 0 : colorOffset.hsv) == null ? void 0 : _e.sp) > 0 ? CustomThemeUtils.minmax(((_f = colorOffset == null ? void 0 : colorOffset.hsv) == null ? void 0 : _f.sp) * 1.25, (_g = colorOffset == null ? void 0 : colorOffset.hsv) == null ? void 0 : _g.sp, 100) : ((_h = colorOffset == null ? void 0 : colorOffset.hsv) == null ? void 0 : _h.sp) * .5
					}
				};
				break;
			case "light":
				result = {
					...
					offset: {
						...colorOffset.hsl,
						lp: ((_i = colorOffset == null ? void 0 : colorOffset.hsl) == null ? void 0 : _i.lp) > 0 ? CustomThemeUtils.minmax((_j = colorOffset == null ? void 0 : colorOffset.hsl) == null ? void 0 : _j.lp, (_k = colorOffset == null ? void 0 : colorOffset.hsl) == null ? void 0 : _k.lp, 100) : ((_l = colorOffset == null ? void 0 : colorOffset.hsl) == null ? void 0 : _l.lp) * .2
					}
				};
				break;
			case "contrast":
	...
	static pattern2ThemeData(pattern) {
		const themeData = {};
		Object.keys(pattern).forEach((colorName2) => {
	...
	static getColorValue(source, offset, mode) {
		...
		const key$1 = mode.split("");
		const value = {
			[key$1[0]]: (sourceModel.color[0] + offset[key$1[0]]) % 360,
			[key$1[1]]: offset[key$1[1] + "p"] > 0 ? sourceModel.color[1] + offset[key$1[1] + "p"] * (100 - sourceModel.color[1]) / 100 : sourceModel.color[1] + sourceModel.color[1] * offset[key$1[1] + "p"] / 100,
			[key$1[2]]: offset[key$1[2] + "p"] > 0 ? sourceModel.color[2] + offset[key$1[2] + "p"] * (100 - sourceModel.color[2]) / 100 : sourceModel.color[2] + sourceModel.color[2] * offset[key$1[2] + "p"] / 100,
...
MediaQueryDispatch$1.prototype = {
	...
	register: function(q, options, shouldDegrade) {
		...
		if (isFunction$1(options)) options = { match: options };
		if (!isArray(options)) options = [options];
		each(options, function(handler) {
			if (isFunction$1(handler)) handler = { match: handler };
...
var src = new MediaQueryDispatch$1();
...
var _PrefersColorSchemeMediaQuery = class {
	...
	register() {
		_PrefersColorSchemeMediaQuery.enquire.register.bind(enquire)(_PrefersColorSchemeMediaQuery.Query.light, { match: () => {
			this.handleColorSchemeChange("light");
		} }).register(_PrefersColorSchemeMediaQuery.Query.dark, { match: () => {
...
var EventBus = class {
	...
	isKeyValueObjInArr(arr, key$1, val) {
		return arr.filter((entry) => {
			return entry[key$1] === val;
		}).length > 0;
	}
	removeFuncInFuncArr(arr, fn) {
		for (let z = 0; z < arr.length; z++) if (this.areFuncEqual(arr[z], fn)) arr.splice(z, 1);
		return arr;
	}
	getKeyValueObjInArr(arr, key$1, val) {
		return arr.filter((entry) => {
			return entry[key$1] === val;
		})[0];
	}
	addEvent(eventName, eventFunc) {
		if (!this.isKeyValueObjInArr(this.eventBusCore, "eventName", eventName)) this.eventBusCore.push({
			eventName,
			eventFuncArr: [eventFunc]
		});
		else this.eventBusCore = this.eventBusCore.map((event) => {
			if (event["eventName"] === eventName) event.eventFuncArr.push(eventFunc);
			return event;
		});
	}
	add(eventName, callbacks) {
		if (!eventName) return;
		if (typeof callbacks === "function") for (let i = 1; i < arguments.length; i++) this.addEvent(eventName, arguments[i]);
		if (typeof callbacks === "object" && callbacks.forEach) callbacks.forEach((fn) => {
			this.addEvent(eventName, fn);
		});
	}
	remove(eventName, callbacks) {
		if (!eventName) return;
		for (let i = 0; i < this.eventBusCore.length; i++) if (this.eventBusCore[i].eventName === eventName) {
			if (arguments.length === 1) return this.eventBusCore.splice(i, 1);
			const removedEvent = this.eventBusCore.splice(i, 1)[0];
			if (typeof callbacks === "function") for (let k = 1; k < arguments.length; k++) removedEvent.eventFuncArr = this.removeFuncInFuncArr(removedEvent.eventFuncArr, arguments[k]);
			if (typeof callbacks === "object" && callbacks.length) for (let x = 0; x < callbacks.length; x++) removedEvent.eventFuncArr = this.removeFuncInFuncArr(removedEvent.eventFuncArr, callbacks[x]);
			this.eventBusCore.push(removedEvent);
		}
	}
	trigger(eventName, data) {
		const event = this.getKeyValueObjInArr(this.eventBusCore, "eventName", eventName);
		if (event) (event.eventFuncArr || []).forEach((fn) => {
			fn.apply(this, data);
		});
	}
};
...
var StorageService = class {
	tryGetLocalStorage(key$1) {
		if (typeof window === "undefined") return null;
		return window.localStorage.getItem(key$1);
	}
	trySetLocalStorage(key$1, value) {
		if (typeof window === "undefined") return;
		window.localStorage.setItem(key$1, value);
	}
};
var ThemeService = class {
	...
	set appendClasses(classes) {
		if (this._appendedClasses) this.removeAppendedClass(this._appendedClasses);
		if (classes) this.addAppendClass(classes);
	...
	initializeTheme(specificThemeId, allowDynamicTheme) {
		...
		if (themeId) {
			...
			if (themes && Object.keys(themes).length > 0) theme = themes[themeId];
		}
		this.currentTheme = theme || infinityTheme;
		this.createColorTransition();
		if (!theme && allowDynamicTheme) return;
	...
	applyTheme(theme) {
		...
		if (!this.contentElement) {
			...
			if (styleElement) this.contentElement = styleElement;
			else {
	...
	setExtraData(data, apply = false) {
		this.extraData = data;
		if (apply) this.applyExtraData();
	}
	applyExtraData() {
		const theme = this.currentTheme;
		if (this.extraData && this.extraData[theme.id] && this.extraData[theme.id].cssVariables) this.contentElement.innerText = ":root { " + this.formatCSSVariables(theme.data) + " }:root { " + this.formatCSSVariables(this.extraData[theme.id].cssVariables) + " }";
		if (this.extraData && this.extraData[theme.id] && this.extraData[theme.id].appendClasses) this.appendClasses = this.extraData[theme.id].appendClasses;
		else this.appendClasses = void 0;
	}
	unloadTheme() {
		if (this.contentElement && document.contains(this.contentElement)) this.contentElement.parentElement.removeChild(this.contentElement);
		if (this.appendClasses) this.appendClasses = void 0;
	}
	registerMediaQuery() {
		if (!this.mediaQuery) this.mediaQuery = new PrefersColorSchemeMediaQuery();
...
function ThemeServiceInit(themes, defaultThemeName, extraData, ieSupport = false, allowDynamicTheme = false) {
	...
	const themeService = new ThemeService(window["globalEventBus"] || new EventBus());
	window[THEME_KEY.themeService] = themeService;
	themeService.setExtraData(extraData || { "devui-dark-theme": { appendClasses: ["dark-mode"] } });
	themeService.initializeTheme(null, allowDynamicTheme);
	if (ieSupport) ieSupportCssVar();
	return themeService;
}
function ThemeServiceFollowSystemOn(themeConfig) {
	...
	return themeService.mediaQuery.prefersColorSchemeChange.subscribe((value) => {
		if (value === "dark") themeService.applyTheme(window[THEME_KEY.themeCollection][themeConfig && themeConfig.darkThemeName || "devui-dark-theme"]);
		else themeService.applyTheme(window[THEME_KEY.themeCollection][themeConfig && themeConfig.lightThemeName || "devui-light-theme"]);
	});
}
function ThemeServiceFollowSystemOff(sub) {
	if (typeof window === "undefined") return null;
	if (sub) sub.unsubscribe();
	window[THEME_KEY.themeService].unregisterMediaQuery();
}
function ieSupportCssVar() {
	if (typeof window === "undefined") return null;
	if (window["CSS"] && CSS.supports && CSS.supports("(--a: 0)") || false) return;
	cssVars({
		watch: true,
		silent: true
	});
...
//#endregion
export { ColorHierarchyMap, ContextService, CustomThemeUtils, EventBus, StorageService, Theme, ThemeService, ThemeServiceFollowSystemOff, ThemeServiceFollowSystemOn, ThemeServiceInit, deepTheme, devuiDarkTheme, devuiGreenDarkTheme, devuiGreenTheme, devuiLightTheme, galaxyTheme, ieSupportCssVar, infinityTheme, provenceTheme, sweetTheme };
