import { t as __commonJSMin } from "./chunk-_n6IbMI2.js";

//#region node_modules/codemirror/lib/codemirror.js
var require_codemirror = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	(function(global, factory) {
		...
		function classTest(cls) {
			return /* @__PURE__ */ new RegExp("(^|\\s)" + cls + "(?:$|\\s)\\s*");
		}
		...
		Delayed.prototype.onTimeout = function(self$1) {
			self$1.id = 0;
			if (self$1.time <= +/* @__PURE__ */ new Date()) self$1.f();
			else setTimeout(self$1.handler, self$1.time - +/* @__PURE__ */ new Date());
		};
		...
		function clipPosArray(doc, array) {
			var out = [];
			for (var i$3 = 0; i$3 < array.length; i$3++) out[i$3] = clipPos(doc, array[i$3]);
			return out;
		}
		...
		function extractLineClasses(type, output) {
			if (type) for (;;) {
				...
				else if (!(/* @__PURE__ */ new RegExp("(?:^|\\s)" + lineClass[2] + "(?:$|\\s)")).test(output[prop$1])) output[prop$1] += " " + lineClass[2];
			}
			return type;
		}
		...
		function widgetTopHeight(lineObj) {
			var height = 0;
			if (lineObj.widgets) {
				for (var i$3 = 0; i$3 < lineObj.widgets.length; ++i$3) if (lineObj.widgets[i$3].above) height += widgetHeight(lineObj.widgets[i$3]);
			}
			return height;
		}
		...
		function cursorCoords(cm, pos, context, lineObj, preparedMeasure, varHeight) {
			...
			function getBidi(ch$1, partPos$1, invert) {
				var right = order[partPos$1].level == 1;
		...
		function prepareSelection(cm, primary) {
			...
			for (var i$3 = 0; i$3 < doc.sel.ranges.length; i$3++) {
				if (!primary && i$3 == doc.sel.primIndex) continue;
				var range$1 = doc.sel.ranges[i$3];
		...
		function drawSelectionCursor(cm, head, output) {
			...
			if (/\bcm-fat-cursor\b/.test(cm.getWrapperElement().className)) {
				...
				if (charPos.right - charPos.left > 0) cursor.style.width = charPos.right - charPos.left + "px";
			}
			if (pos.other) {
		...
		function drawSelectionRange(cm, range$1, output) {
			...
			function drawForLine(line, fromArg, toArg) {
				...
				function wrapX(pos, dir, side) {
					...
					return coords(side == "after" ? extent.begin : extent.end - (/\s/.test(lineObj.text.charAt(extent.end - 1)) ? 2 : 1), prop$1)[prop$1];
				}
		...
		NativeScrollbars.prototype.enableZeroWidthBar = function(bar, delay, type) {
			...
			function maybeDisable() {
				...
				if ((type == "vert" ? document.elementFromPoint(box.right - 1, (box.top + box.bottom) / 2) : document.elementFromPoint((box.right + box.left) / 2, box.bottom - 1)) != bar) bar.style.pointerEvents = "none";
		...
		function computeReplacedSel(doc, changes, hint) {
			...
			for (var i$3 = 0; i$3 < changes.length; i$3++) {
				...
				newPrev = to;
				if (hint == "around") {
					var range$1 = doc.sel.ranges[i$3], inv = cmp(range$1.head, range$1.anchor) < 0;
		...
		function linkedDocs(doc, f, sharedHistOnly) {
			function propagate(doc$1, skip, sharedHist) {
				if (doc$1.linked) for (var i$3 = 0; i$3 < doc$1.linked.length; ++i$3) {
					var rel = doc$1.linked[i$3];
					if (rel.doc == skip) continue;
					var shared = sharedHist && rel.sharedHist;
					if (sharedHistOnly && !shared) continue;
					f(rel.doc, shared);
					propagate(rel.doc, doc$1, shared);
				}
			}
		...
		function historyChangeFromChange(doc, change) {
			...
			linkedDocs(doc, function(doc$1) {
				return attachLocalSpans(doc$1, histChange, change.from.line, change.to.line + 1);
			}, true);
			return histChange;
		}
		...
		function extendSelections(doc, heads, options) {
			...
			for (var i$3 = 0; i$3 < doc.sel.ranges.length; i$3++) out[i$3] = extendRange(doc.sel.ranges[i$3], heads[i$3], null, extend);
			setSelection(doc, normalizeSelection(doc.cm, out, doc.sel.primIndex), options);
		}
		function replaceOneSelection(doc, i$3, range$1, options) {
		...
		function filterSelectionChange(doc, sel, options) {
			var obj = {
				...
				update: function(ranges) {
					this.ranges = [];
					for (var i$3 = 0; i$3 < ranges.length; i$3++) this.ranges[i$3] = new Range(clipPos(doc, ranges[i$3].anchor), clipPos(doc, ranges[i$3].head));
				},
		...
		function skipAtomicInSelection(doc, sel, bias, mayClear) {
			...
			for (var i$3 = 0; i$3 < sel.ranges.length; i$3++) {
				...
				var old = sel.ranges.length == doc.sel.ranges.length && doc.sel.ranges[i$3];
				var newAnchor = skipAtomic(doc, range$1.anchor, old && old.anchor, bias, mayClear);
				var newHead = skipAtomic(doc, range$1.head, old && old.head, bias, mayClear);
		...
		function makeChange(doc, change, ignoreReadOnly) {
			...
			if (split) for (var i$3 = split.length - 1; i$3 >= 0; --i$3) makeChangeInner(doc, {
				from: split[i$3].from,
				to: split[i$3].to,
		...
		function makeChangeInner(doc, change) {
			...
			var rebased = [];
			linkedDocs(doc, function(doc$1, sharedHist) {
				if (!sharedHist && indexOf(rebased, doc$1.history) == -1) {
					rebaseHist(doc$1.history, change);
					rebased.push(doc$1.history);
				}
				makeChangeSingleDoc(doc$1, change, null, stretchSpansOverChange(doc$1, change));
			});
		}
		function makeChangeFromHistory(doc, type, allowSelectionOnly) {
			...
			var loop = function(i$4) {
				...
				makeChangeSingleDoc(doc, change, i$4 ? computeSelAfterChange(doc, change) : lst(source), mergeOldSpans(doc, change));
				if (!i$4 && doc.cm) doc.cm.scrollIntoView({
					from: change.from,
					to: changeEnd(change)
				});
				var rebased = [];
				linkedDocs(doc, function(doc$1, sharedHist) {
					...
					makeChangeSingleDoc(doc$1, change, null, mergeOldSpans(doc$1, change));
				});
			};
		...
		function shiftDoc(doc, distance) {
			...
			doc.first += distance;
			doc.sel = new Selection(map(doc.sel.ranges, function(range$1) {
		...
		function markTextShared(doc, from, to, options, type) {
			...
			linkedDocs(doc, function(doc$1) {
				...
				markers.push(markText(doc$1, clipPos(doc$1, from), clipPos(doc$1, to), options, type));
				for (var i$3 = 0; i$3 < doc$1.linked.length; ++i$3) if (doc$1.linked[i$3].isParent) return;
				primary = lst(markers);
			});
		...
		Doc.prototype = createObj(BranchChunk.prototype, {
			...
			unlinkDoc: function(other) {
				...
				if (this.linked) for (var i$3 = 0; i$3 < this.linked.length; ++i$3) {
					if (this.linked[i$3].doc != other) continue;
		...
		function leftButtonSelect(cm, event, start, behavior) {
			...
			function extendTo(pos) {
				...
				if (behavior.unit == "rectangle") {
					...
					setSelection(doc, normalizeSelection(cm, startSel.ranges.slice(0, ourIndex).concat(ranges$1), ourIndex), {
						origin: "*mouse",
						scroll: false
					});
					...
					ranges$1$1[ourIndex] = bidiSimplify(cm, new Range(clipPos(doc, anchor), head));
					setSelection(doc, normalizeSelection(cm, ranges$1$1, ourIndex), sel_mouse);
				}
			}
		...
		var defaults = {};
		var optionHandlers = {};
		function defineOptions(CodeMirror$1) {
			var optionHandlers$1 = CodeMirror$1.optionHandlers;
			function option(name, deflt, handle, notOnInit) {
				CodeMirror$1.defaults[name] = deflt;
				if (handle) optionHandlers$1[name] = notOnInit ? function(cm, val, old) {
					if (old != Init) handle(cm, val, old);
				} : handle;
			}
			CodeMirror$1.defineOption = option;
			CodeMirror$1.Init = Init;
		...
		function indentLine(cm, n, how, aggressive) {
			...
			if (indentString != curSpaceString) {
				...
				return true;
			} else for (var i$1$1 = 0; i$1$1 < doc.sel.ranges.length; i$1$1++) {
				var range$1 = doc.sel.ranges[i$1$1];
				if (range$1.head.line == n && range$1.head.ch < curSpaceString.length) {
					var pos$1 = Pos(n, curSpaceString.length);
					replaceOneSelection(doc, i$1$1, new Range(pos$1, pos$1));
					break;
				}
			}
		}
			...
			if (paste && sel.ranges.length > 1) {
				if (lastCopied && lastCopied.text.join("\n") == inserted) {
					if (sel.ranges.length % lastCopied.text.length == 0) {
						multiPaste = [];
						for (var i$3 = 0; i$3 < lastCopied.text.length; i$3++) multiPaste.push(doc.splitLines(lastCopied.text[i$3]));
					}
		...
		function addEditorMethods(CodeMirror$1) {
			var optionHandlers$1 = CodeMirror$1.optionHandlers;
			var helpers = CodeMirror$1.helpers = {};
			CodeMirror$1.prototype = {
				constructor: CodeMirror$1,
				focus: function() {
					window.focus();
				...
				addOverlay: methodOp(function(spec, options) {
					var mode = spec.token ? spec : CodeMirror$1.getMode(this.options, spec);
				...
				getModeAt: function(pos) {
					...
					return CodeMirror$1.innerMode(mode, this.getTokenAt(pos).state).mode;
				},
				...
				moveV: methodOp(function(dir, unit) {
					...
					if (goals.length) for (var i$3 = 0; i$3 < doc.sel.ranges.length; i$3++) doc.sel.ranges[i$3].goalColumn = goals[i$3];
				}),
			...
			eventMixin(CodeMirror$1);
			CodeMirror$1.registerHelper = function(type, name, value) {
				if (!helpers.hasOwnProperty(type)) helpers[type] = CodeMirror$1[type] = { _global: [] };
				helpers[type][name] = value;
			};
			CodeMirror$1.registerGlobalHelper = function(type, name, predicate, value) {
				CodeMirror$1.registerHelper(type, name, value);
		...
		TextareaInput.prototype.focus = function() {
			if (this.cm.options.readOnly != "nocursor" && (!mobile || activeElt() != this.textarea)) try {
				this.textarea.focus();
			} catch (e) {}
		};
		...
		function addLegacyProps(CodeMirror$1) {
			CodeMirror$1.off = off;
			CodeMirror$1.on = on;
			CodeMirror$1.wheelEventPixels = wheelEventPixels;
			CodeMirror$1.Doc = Doc;
			CodeMirror$1.splitLines = splitLinesAuto;
			CodeMirror$1.countColumn = countColumn;
			CodeMirror$1.findColumn = findColumn;
			CodeMirror$1.isWordChar = isWordCharBasic;
			CodeMirror$1.Pass = Pass;
			CodeMirror$1.signal = signal;
			CodeMirror$1.Line = Line;
			CodeMirror$1.changeEnd = changeEnd;
			CodeMirror$1.scrollbarModel = scrollbarModel;
			CodeMirror$1.Pos = Pos;
			CodeMirror$1.cmpPos = cmp;
			CodeMirror$1.modes = modes;
			CodeMirror$1.mimeModes = mimeModes;
			CodeMirror$1.resolveMode = resolveMode;
			CodeMirror$1.getMode = getMode;
			CodeMirror$1.modeExtensions = modeExtensions;
			CodeMirror$1.extendMode = extendMode;
			CodeMirror$1.copyState = copyState;
			CodeMirror$1.startState = startState;
			CodeMirror$1.innerMode = innerMode;
			CodeMirror$1.commands = commands;
			CodeMirror$1.keyMap = keyMap;
			CodeMirror$1.keyName = keyName;
			CodeMirror$1.isModifierKey = isModifierKey;
			CodeMirror$1.lookupKey = lookupKey;
			CodeMirror$1.normalizeKeyMap = normalizeKeyMap;
			CodeMirror$1.StringStream = StringStream;
			CodeMirror$1.SharedTextMarker = SharedTextMarker;
			CodeMirror$1.TextMarker = TextMarker;
			CodeMirror$1.LineWidget = LineWidget;
			CodeMirror$1.e_preventDefault = e_preventDefault;
			CodeMirror$1.e_stopPropagation = e_stopPropagation;
			CodeMirror$1.e_stop = e_stop;
			CodeMirror$1.addClass = addClass;
			CodeMirror$1.contains = contains;
			CodeMirror$1.rmClass = rmClass;
			CodeMirror$1.keyNames = keyNames;
		}
...
//#endregion
export { require_codemirror as t };
//# sourceMappingURL=codemirror-DdPLNu_x.js.map